-- Atomic RPC: save panel layout (update metadata + replace rows + replace ports in one transaction)
...
-- Atomic RPC: create panel layout with default rows in one transaction
...
declare
  new_id uuid;
begin
  insert into panel_layouts (project_id, name, height_ru, facing, has_lacing_bar, notes, weight_kg)
  values (p_project_id, p_name, p_height_ru, p_facing, p_has_lacing_bar, p_notes, p_weight_kg)
...
-- Atomic RPC: duplicate panel layout (panel + rows + ports in one transaction)
...
begin
  ...
  insert into panel_layout_ports (panel_layout_id, connector_id, row_index, hole_index, span_w, span_h, label)
  select new_id, connector_id, row_index, hole_index, span_w, span_h, label
