  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {devices.map((device) => {
        ...
        return (
          <div key={device.id} className="border rounded-lg p-4 bg-white hover:shadow-sm transition-shadow">
            {thumbUrl && (
              <img
                src={thumbUrl}
                alt={`${device.brand} ${device.model}`}
                className="w-full h-24 object-contain bg-gray-100 rounded mb-3"
              />
            )}
            {!thumbUrl && (
